...
@section('page_title','Guru | Tendik')
        ...
        <table class="table align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="table-title">
                                ...
                                <small>{{ ucfirst($member->nip) }} </small>
                                <small class="d-block text-secondary">{{ $member->simpeg_email ?: '-' }}</small>                           
                                <small class="d-block">{{ $member->simpeg_no_hp ?: 'No. HP belum tersedia' }}</small>                                
                            </div>
                        </div>
                    </td>
                    <td>
                        @if($member->simpeg_employee_id)
                        <strong>{{ $member->simpeg_status_pegawai ?: 'Status belum tersedia' }} | <small>{{ ucfirst($member->position) }}</small></strong>
                    ...
                    <td>
                       <small><center>{{ $member->simpeg_employee_id ? 'Terhubung SIMPEG' : 'Data lokal' }}</center></small>
                        @if($member->user)
                        ...
                        @else
                        <span class="badge bg-light text-secondary">Penulis : Belum Terhubung</span>
                        @endif
                    </td>
                    <td>
                        <small class="d-block text-secondary">{{ \Carbon\Carbon::parse($member->simpeg_synced_at)->format('d/m/Y H:i') }}</small>
                        @if($member->simpeg_synced_at)@endif
                    </td>
                    <td>
                        <span class="status-dot {{ $member->active ? 'published' : 'draft' }}">{{ $member->active ? 'Tampil ' : 'Tidak' }}</span>
